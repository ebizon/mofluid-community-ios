...
//  AJAlertController
//
//  Created by Saurabh on 13/12/17.
...
class AJAlertController: UIViewController {
    ...
    private var strAlertTitle = "Mofluid!"
    ...
    {
        if let appDelegate = UIApplication.shared.delegate, let window = appDelegate.window, let _ = window?.rootViewController {
            
            var topViewController = vc
    ...
    public func showAlertWithOkButton( aStrMessage:String,vc:UIViewController,
                                completion : alertCompletionBlock){
